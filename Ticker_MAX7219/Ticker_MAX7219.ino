  ...
  */
  ...
  else if (btMode == 1)
  {
    ...
    else if (btStatic == 1 && displayChanged)
    {
      ...
      //Отрисовываем пока не дойдем до правого края матрицы
      while (col < 32)
      {
        //Если символы в строке остались заполняем символами, иначе пустотой
        if (symbNumber < btString.length())
        {
          //Находим индекс текущего символа в массиве шрифтов
          if (btString[symbNumber] < 127)
          {
            fontIndex = btString[symbNumber] - 32; 
          }
          else if (btString[symbNumber] > 191)
          {
          ...
          for (int i = 0; i < 5; i++)
          {
            //если с учётом офсета не дошли до конца матрицы, то продолжаем
            if ((col + i) < 32)
            {
              for (int j = 0; j < 8; j++)
              {
                if (font[fontIndex][i] & (1 << j))
                  matrix.drawPixel(col + i, j, HIGH);
                else
                  matrix.drawPixel(col + i, j, LOW);
              }
            }
          }
          
          //Офсет по отрисованному символу (+5) и промежутку(+1)
          col += 6;

          //Переходим по строке к следующему символу
          symbNumber++;
        }
        else 
          for (int i = 0; i < 8; i++)
            matrix.drawPixel(col, i, LOW);
        col++;
      }
